...
This requirements class establishes the Raster Space used. There are currently only two options: _RasterPixelIsPoint_ and _RasterPixelIsArea_. No user-defined raster spaces are currently supported. For variance in imaging display parameters, such as pixel aspect-ratios, use the standard TIFF 6.0 device-space tags.
