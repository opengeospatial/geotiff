...
But even for these cases, unless stated through a Citation, some assumption is required about axis positive direction and axis order, based on the values for the transformation from raster space to model space. With no negative values in the _ModelPixelScaleTag_ it is assumed that:
